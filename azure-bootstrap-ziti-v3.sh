...
LE_EMAIL="${4:?missing Let's Encrypt email}"
...
echo "[INFO] Install NGINX/Let's Encrypt: ${INSTALL_NGINX_LE}"
...
chmod +x install-openziti-v2-controller-zac.sh install-nginx-letsencrypt-zac-fix.sh
...
echo "[INFO] Writing helper to rerun NGINX/Let's Encrypt later"
...
if [[ "${INSTALL_NGINX_LE}" == "true" ]]; then
  echo "[INFO] Installing NGINX/Let's Encrypt/ZAC fix"
  DOMAIN_NAME="${AZURE_FQDN}" \
  ADMIN_EMAIL="${LE_EMAIL}" \
  ./install-nginx-letsencrypt-zac-fix.sh
else
  echo "[INFO] Skipping NGINX/Let's Encrypt because installNginxLetsEncrypt=false"
fi
